...
static bool Web_IsValidOledMenuItems(const char *value) {
	...
	static constexpr const char *knownItems[] = {"status", "equalizer", "nightmode", "bluetooth", "fwupdate", "shutdown"};
	...
	for (char *token = strtok_r(copy, ",", &save); token != nullptr; token = strtok_r(nullptr, ",", &save)) {
		...
		// Accept cached/legacy clients and treat their retired WebDAV slot as Bluetooth.
		const char *canonicalToken = strcmp(token, "webdav") == 0 ? "bluetooth" : token;
		bool found = false;
		for (size_t i = 0; i < sizeof(knownItems) / sizeof(knownItems[0]); i++) {
			if (!seen[i] && strcmp(canonicalToken, knownItems[i]) == 0) {
				seen[i] = true;
				found = true;
				break;
			}
...
// settingsToJSON + the get-settings/eq-rule/operation-mode handlers are declared in WebInternal.h
// (defined in WebSettings.cpp). JSONToSettings (the websocket dispatcher) stays here.
...
void Web_Exit(void) {
	esp_task_wdt_reset();
	if (webserverStarted) {
		WebExplorer_AbortUpload(); // gracefully abort an active file-storage task and wait for it to release its files
...
void webserverStart(void) {
	if (!webserverStarted && (Wlan_IsConnected() || (WiFi.getMode() == WIFI_AP))) {
		...
		wServer.on("/syncreport", HTTP_GET, [](AsyncWebServerRequest *request) {
			// Don't serve the report while a sync is running: the sync task (core 1) is still writing
			// (and may truncate-reopen) the file, so a concurrent stream would return a partial report.
			if (Sync_GetStatus() == 1) {
				request->send(409, "text/plain; charset=utf-8", "sync still running");
				return;
			}
		...
		wServer.on("/version", HTTP_GET, [](AsyncWebServerRequest *request) {
			...
			char latest[24];
			Web_GetLatestBuild(latest, sizeof(latest));
			char buf[160];
			snprintf(buf, sizeof(buf), "{\"build\":\"%s\",\"upToDate\":%d,\"latest\":\"%s\"}", buildRevision, (int) Web_GetFirmwareUpToDate(), latest);
			request->send(200, "application/json", buf);
		});
		...
		wServer.on("/rfidsync", HTTP_GET, [](AsyncWebServerRequest *request) {
			char msg[96];
			RfidSync_CopyMessage(msg, sizeof(msg));
			Web_SendStatusJson(request, RfidSync_GetStatus(), -1, msg);
		});
		...
		wServer.on("/currenttrack", HTTP_GET, [](AsyncWebServerRequest *request) {
			...
			if (active) {
				// The audio task (core 1) can free + reassign the playlist while we read it here on the
				// web task (core 0). Take the playlist lock, copy the path out into a String, then
				// release before any SD I/O (open()) so we never dereference a freed vector/string.
				String trackPath;
				bool havePath = false;
...
WebsocketCodeType JSONToSettings(JsonObject doc) {
	...
	if (doc["general"].is<JsonObject>()) {
		...
		if (!generalObj["seekStep"].isNull()) { // step (s) for smart forward/backward in-file seeking
			...
			if (seekStep == 0) {
				seekStep = seekStepDefault; // 0 would silently disable smart-seek; the UI enforces a min but REST/MQTT might send 0
			}
	...
	if (doc["oled"].is<JsonObject>()) {
		...
		success = success && (gPrefsSettings.putBool("oledShowArtist", oledObj["showArtist"].as<bool>()) != 0);
...
static void handleCoverImageRequest(AsyncWebServerRequest *request) {
	...
	if (!gPlayProperties.coverFilePos || !gPlayProperties.playlist) {
		...
		if (stationLogoUrl != "") {
			...
			if (gPlayProperties.playMode == WEBSTREAM || (gPlayProperties.playMode == LOCAL_M3U && gPlayProperties.isWebstream)) {
				...
				// no embedded cover -> try a folder cover image (cover.jpg / folder.jpg / ...) next to the track.
				// Copy the track path out under the playlist lock (the audio task may free it on core 1).
				String track;
				AudioPlayer_LockPlaylist();
				if (gPlayProperties.playlist && (gPlayProperties.playMode != NO_PLAYLIST) && (gPlayProperties.currentTrackNumber < gPlayProperties.playlist->size())) {
					track = gPlayProperties.playlist->at(gPlayProperties.currentTrackNumber);
				}
				AudioPlayer_UnlockPlaylist();
				if (track.length() > 0) {
					int slash = track.lastIndexOf('/');
					if (slash >= 0) {
						const String dir = track.substring(0, slash);
						static const char *coverNames[] = {"/cover.jpg", "/cover.jpeg", "/cover.png", "/folder.jpg", "/folder.png"};
						for (const char *name : coverNames) {
							const String coverPath = dir + name;
							if (gFSystem.exists(coverPath)) {
								const char *mime = coverPath.endsWith(".png") ? "image/png" : "image/jpeg";
								Log_Printf(LOGLEVEL_NOTICE, "serve folder cover: '%s'", coverPath.c_str());
								request->send(gFSystem, coverPath, mime);
								return;
							}
						}
					}
	...
	// copy the path out under the playlist lock so the audio task (core 1) can't free it while the
	// rest of this handler uses it across SD I/O below (use-after-free otherwise)
	String coverFileName;
	AudioPlayer_LockPlaylist();
	if (gPlayProperties.playlist && (gPlayProperties.currentTrackNumber < gPlayProperties.playlist->size())) {
		const char *p = gPlayProperties.playlist->at(gPlayProperties.currentTrackNumber);
		if (p) {
			coverFileName = p;
		}
	}
