...
void RfidPn5180_Task(void *parameter) {
	...
	for (;;) {
		...
		if (gPlayProperties.pauseIfRfidRemoved) {
			...
			if (!cardAppliedCurrentRun && cardAppliedLastRun && !gPlayProperties.pausePlay && !gPlayProperties.playlistFinished && gPlayProperties.playMode != NO_PLAYLIST && System_GetOperationMode() != OPMODE_BLUETOOTH_SINK) { // Card removed => pause or stop
