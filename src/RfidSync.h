...
// Wire format matches the RFID store (server/rfid.php): entries are {id, timestamp, fileOrUrl,
// playMode} for music cards or {id, timestamp, modId} for modification cards.
// Settings (NVS): rfidSyncUrl (server endpoint, e.g. .../rfid/rfid.php), reuses syncUser/syncPwd for
// auth, rfidPeers (JSON array of peer host/key objects; legacy lists are accepted), rfidSyncLearn.
