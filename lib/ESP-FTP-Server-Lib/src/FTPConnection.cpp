...
// ESPuino fork: accept ANY username. The username is irrelevant - only the shared device
// password (set on the Security tab) is checked in c_PASS().
void FTPConnection::c_USER() {
  _AuthUsername = _LineSplited[1];
...
void FTPConnection::c_PASS() {
  ...
  // ESPuino fork: ignore the username and match only the password against any configured user.
  // An empty configured password means the server is open (any password is accepted).
  std::list<FTPUser>::iterator _user = std::find_if(_UserList.begin(), _UserList.end(), [&](const FTPUser &user) {
    return user.Password.isEmpty() || password == user.Password;
  });
