...
BINARY_FILES = [
    ...
    # vendored third-party libs bundled into one JS + one CSS request (regenerate
    # with tools/bundle_assets.py); served locally so the UI works without internet.
    # Bundling keeps the request count low so ESPAsyncWebServer / Safari don't stall.
    Path("js/vendor.min.js"),
    Path("css/vendor.min.css"),
    # binary assets referenced by the bundled CSS (font-awesome + jstree sprites + fonts)
