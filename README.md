...
| Password protection: single password (no username), 90-day session cookie, logout menu entry, brute-force lockout; inactive in hotspot mode. API clients/scripts can also authenticate per-request via **HTTP Basic Auth** (username `espuino`, password = the web password, e.g. `curl -u espuino:<pw>`) instead of the cookie/login flow | [`e74e712`](../../commit/e74e712) |
...
| **OpenAPI/Swagger spec updated**: the bundled `REST_API.yaml` (served at `/swagger.html`) now documents all fork-added endpoints (`/sync`, `/syncstop`, `/playlist`, `/githubupdate`, `/version`, `/login`, `/logout`, `/security`, `/eqrule(s)`, `/activeequalizer`, `/bluetooth*`, `/sdclean`) and declares a `basicAuth` security scheme so the Swagger "Authorize" button works (username `espuino`) | [`f8298d9`](../../commit/f8298d9) |
