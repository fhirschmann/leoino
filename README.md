...
| **OLED quick menu** (`CMD_OLED_MENU`, command 194): assign it to button 6 (or any physical/IR button) in the web UI. One short press opens the menu, the volume encoder browses without changing volume, and the same button confirms. Under Settings → OLED, entries can be enabled/hidden and drag-sorted, the 1–30 s inactivity timeout is configurable, and the last selection can optionally be remembered. One consolidated status page shows playback/volume, IP, battery, firmware, uptime, heap and SD health; list rows also show live EQ, night-mode, Bluetooth mode and firmware-update state. The Equalizer entry opens its own encoder-driven submenu for Flat, Music, Speech and Voice Boost; the Bluetooth submenu selects Normal, Speaker or Headphones mode. Pressing again applies the selected value; a changed Bluetooth mode restarts the device through the existing safe mode-switch path. Firmware update reports **UP TO DATE** without reflashing when the latest release is already installed; actual updates and shutdown still require a second confirmation press. Long-press and rotary-modifier assignments are unchanged | [`feature`](../../commits/master) |
