...
| **OLED quick menu** (`CMD_OLED_MENU`, command 194): assign it to button 6 (or any physical/IR button) in the web UI. One short press opens the menu, the volume encoder browses without changing volume, and the same button confirms. It closes after 5 seconds without interaction. Built-in entries show system status, IP address and battery details or execute equalizer, night mode, WebDAV, firmware update and shutdown through the existing command paths; long-press and rotary-modifier assignments are unchanged | [`feature`](../../commits/master) |
