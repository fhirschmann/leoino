...
| **OLED quick menu** (`CMD_OLED_MENU`, command 194): assign it to button 6 (or any physical/IR button) in the web UI. One short press opens the menu, the volume encoder browses without changing volume, and the same button confirms. Under Settings → OLED, entries can be enabled/hidden and drag-sorted, the 1–30 s inactivity timeout is configurable, and the last selection can optionally be remembered. List rows show live playback/IP/battery, EQ, night-mode and WebDAV state; detail pages cover status, IP, battery and system information (firmware, uptime, heap and SD health). Firmware update and shutdown require a second confirmation press and can be cancelled by turning or waiting. All operational entries still use the existing command paths; long-press and rotary-modifier assignments are unchanged | [`feature`](../../commits/master) |
