...
| **Settings tabs de-cluttered**: the long "General" sub-tab was split — the **LED** settings (NeoPixel brightness + LED config) moved into a new **LEDs** sub-tab, and **Deep sleep + Battery** into a new **Power** sub-tab. Both new tabs are covered by the same auto-save. General now holds just volume/branding/options/playlist/rotary. i18n de/en/fr | [`settings-split`](../../releases/tag/settings-split) |
| **Folder cover-art fallback**: when a track has no embedded ID3 cover, `/cover` now serves a sidecar image from the track's folder (`cover.jpg`/`.jpeg`/`.png` or `folder.jpg`/`.png`) before falling back to the placeholder icon — so audiobooks/albums with a folder image show art in the Control tab | [`cover-fallback`](../../releases/tag/cover-fallback) |
| **Most-played cards (listening stats)**: a per-card play counter (incremented when a music card starts a playlist) drives a **most-played** list — the top 3 cards (path + play count) appear in the info dialog next to the daily totals; full top-10 via a new `GET /topcards`. A card's counter is cleared when the card is deleted (no ghost stats). Stored in NVS (`rfidPlayCnt`) | [`stats-topcards`](../../releases/tag/stats-topcards) |
| **MQTT now-playing metadata**: the ID3 **artist** and **album** are published to MQTT state topics (`track_artist`/`track_album`, alongside the existing `track` title and `cover_changed`) and registered as Home-Assistant discovery sensors, so a broker/Home Assistant sees the current artist & album. Reset per track. | — |
