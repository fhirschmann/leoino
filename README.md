...
| **File browser: sizes & assignment badges**: the Files-tab tree now shows the size of every entry right-aligned (folders summed recursively, so a whole audiobook folder shows its total size); the `GET /explorer` response gained a `size` field (bytes) per entry. Each row is also badged in front of the name when that exact path has an **RFID tag** assigned (tag icon) and/or a **per-path EQ rule** (sliders icon), so you can see at a glance what is mapped where. Badges refresh when a folder is (re)opened and update live right after you assign an RFID tag or EQ profile to the selected node | [`PENDING`](../../commit/PENDING) |
