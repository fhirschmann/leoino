...
| **Selectable UI themes** (menu → 🎨): switch the whole interface between the default **Cyberpunk** look and three bright, rounded, playful alternatives — **Kids** (sunny), **Candy** (pink/purple) and **Galaxy** (blue/indigo). Themes are a pure client-side CSS-variable swap (zero extra load on the ESP32 — the device still serves one static page) and the choice is remembered per browser in `localStorage` | [`6bb0097`](../../commit/6bb0097) · [`THEMECOMMIT`](../../commit/THEMECOMMIT) |
