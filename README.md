...
| **File browser: assignment badges + context-menu RFID learn**: in the Files-tab tree each row is badged (right-aligned, after the name) when that exact path has an **RFID tag** assigned (tag icon) and/or a **per-path EQ rule** (sliders icon), so you can see at a glance what is mapped where. The explorer **context menu** now also learns/removes RFID assignments directly: place a card (its id fills the field), then right-click a file/folder → **RFID** → pick a play mode to assign it to that card on the spot (no tab switch / submit), or **remove** an existing assignment. Mirrors the existing EQ-profile submenu. Badges refresh when a folder is (re)opened and live after assigning/removing. i18n de/en/fr | [`a4795bd`](../../commit/a4795bd) |
| **Virtual RFID cards**: assign and play content without a physical tag. A **Virtual card** button next to the RFID-id field generates a random 12-digit id (formatted like a real 4-byte UID) and keeps it in the field (protected from being overwritten by background settings pushes) so it can be assigned via the form or the explorer context menu. A new `POST /rfidtrigger?id=<12 digits>` endpoint "lays" any card id (virtual or physical) — it enqueues the id exactly like a scan, so the assigned playlist/folder starts playing; the saved-assignments list (Tools tab) gets a ▶ button per entry to trigger it from the browser. MQTT can already lay a card via the `rfid` command topic; both now share one helper (`Rfid_QueueCardString`). i18n de/en/fr | [`PENDING`](../../commit/PENDING) |
