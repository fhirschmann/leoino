...
| One-click OTA update: GitHub Actions publishes a rolling `latest` release (`firmware.bin` + `version.json`); the Tools tab checks the device version against it and flashes the new firmware via OTA | [`06ac13d`](../../commit/06ac13d) |
