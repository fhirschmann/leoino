...
- 🎨 **Cyberpunk web interface, six switchable themes** — management + access-point pages
  fully rebuilt in neon; pick between **Cyberpunk** and five kid-friendly looks
  (**Waldhaus**, **Wölkchen**, **Pixelheld**, **Robo**, **Kreidetafel**), remembered per
  browser. A **settings search** box filters every option across all sub-tabs and jumps
...
- 🔄 **HTTP file sync** — pull audio onto the SD card from a manifest, with opt-in **mirror
  mode**, a **dry-run** preview and conflict-safe **two-way playlist sync** between devices.
...
- 💤 **Reliable sleep modes** — the “sleep after 5 tracks” modification card now works and
  is selectable in the UI; MQTT publishes a live `sleep_timer_state` JSON countdown for
  minute-, track- and playlist-based timers.
...
Six selectable themes (menu → 🎨) — each a pure client-side swap of the colour palette plus a
shape/typography "family", remembered per browser. **Cyberpunk** is the default (and the only one
with a dark/light toggle); the five kid-friendly designs each pin their own mood:
...
<td align="center" width="50%"><img src="docs/img/theme-kreidetafel.png" alt="Kreidetafel theme"><br><sub><strong>Kreidetafel</strong> — dark slate chalkboard, hand-drawn, boxless tabs</sub></td>
</tr>
</table>
...
| **Consolidated, auto-saving Settings tab**: WiFi/MQTT/FTP/Bluetooth merged with the general settings into one tab; the sub-tabs are a **vertical UniFi-style sidebar** (icon-only on mobile; stays pinned while scrolling) with flat, consistent sections, split into General / **Buttons** / **LEDs** / **Power** / **RFID** / … . General/Buttons/LEDs/Power/RFID auto-save (debounced); the credential tabs keep an explicit submit on purpose. Physical-button assignments include **Smart forward/backward**: seek within a single long file, or change tracks in a multi-track playlist. A dedicated 100 Hz sampler records real press/release edges even while opening an SD track temporarily blocks the main loop; buffered clicks are dispatched individually, so rapid short taps no longer disappear and the display still shows every intermediate track—without adding an IR/Smart coalescing delay or changing long-press timing. Assigning either Smart command to a button's **long press** makes it auto-repeat every 200 ms while held, matching IR-remote fast scrolling without firing an extra action on release | [`8b01876`](../../commit/8b01876) · [`1944cf4`](../../commit/1944cf4) · [`c284d0e`](../../commit/c284d0e) · [`32aabfd`](../../commit/32aabfd) |
...
| **One-click OTA + version badge**: a Tools-tab button (also bindable command **186** / MQTT `firmware_update`) pulls the rolling `latest` GitHub release and flashes it over OTA; a navbar badge shows the running build and turns amber when an update is available (passive `/version` check) — click it to install. With HomeKit enabled the internal RAM is too tight for the TLS download, so the device transparently **reboots into an update window** first (HomeKit is held back for that one boot, the download runs, then it reboots into the new firmware) | [`8527f5e`](../../commit/8527f5e) · [`b736abc`](../../commit/b736abc) |
| **HTTP file sync + bidirectional playlists**: audio is still pulled from the server onto the SD card from a JSON manifest — additive by default (with an optional **mirror mode**, see below), streamed in chunks straight to SD, background task with live progress + stop, abort-on-button, stall watchdog, auto-pauses playback and keeps the device awake mid-transfer. With the companion manifest v2, `/Playlists/*.m3u[8]` additionally syncs **both ways**: local creates/edits/renames/deletes are uploaded, other ESPuinos pull them on their next sync, same-size edits are detected by SHA-256, and server revisions + persistent tombstones prevent stale devices from resurrecting deletions. Concurrent edits are never silently overwritten — the local version is kept as a named conflict copy. A hidden local base snapshot enables a real three-way merge without trusting device clocks. The manifest remains buffered to SD and parsed **one entry at a time**, so a large manifest (1000+ files) fits in the ESP32's limited heap; old v1 manifests and firmware remain pull-only and compatible | [`ac24bbc`](../../commit/ac24bbc) · [`42d2c46`](../../commit/42d2c46) |
...
| **WebDAV server**: mount the SD card as a network drive (`http://<ip>:81/`) to copy audio on/off it straight from Finder/Explorer — no FTP client needed. Self-contained server (OPTIONS/PROPFIND/GET/HEAD+ranges/PUT/DELETE/MKCOL/MOVE/COPY/LOCK) running in its own task pinned to core 0 so transfers never disturb the audio pipeline; optional HTTP-Basic credentials. Configure + auto-start on boot in its own **WebDAV** settings sub-tab; start/stop live from the Control tab, command **188** or MQTT `webdav` (Home Assistant switch included). With auto-start on, the share is announced over **Bonjour/mDNS** (`_webdav._tcp`, `path=/`) so it pops up by itself in the Finder/Explorer network sidebar — no manual "Connect to Server" needed. The request path is hardened against the crash/robustness issues an adversarial review surfaced: chunked request bodies are drained so Finder doesn't RST-retry-storm; the enable/disable lifecycle and live credential reload are synchronized; request-line/header size is capped so a malformed request can't OOM the ~53 KB internal heap; parallel reads remain possible while PUT/DELETE/MOVE/COPY are mutually exclusive and pause audio before touching FAT | [`9b2bee0`](../../commit/9b2bee0) · [`92ed83b`](../../commit/92ed83b) |
| **RFID-tag syncing (central server + peer-to-peer)**: keep tag assignments in sync across a PHP server *and* other ESPuinos — newest-wins by timestamp, deletions propagate via tombstones (including full peer catch-up after a device was offline), and automatic catch-up retries until its background job actually starts. Stale peer writes cannot overwrite a newer assignment or resurrect a deleted tag. The single-tag push worker now exists only while its queue has work, returning its otherwise permanent 4–8 KB internal stack to the web interface between learns/deletes. Run from the Tools tab, command **187** or MQTT `rfid_sync` | [`99ffb5b`](../../commit/99ffb5b) |
...
| Change | Commit |
| --- | --- |
| **Robust playlist hand-off in the audio state machine**: a newly selected playlist is now published to the audio task together with all of its mode/resume/RFID state under one mutex, so rapid card or WebUI changes cannot mix an old playlist with new metadata or free a playlist while it is being adopted. Cross-task LED/OLED/MQTT/WebSocket readers take safe snapshots, upload pause/resume is executed by the audio task itself, and a newly arriving transport command can no longer be erased while the preceding one is handled. Saved track numbers are range-checked, empty playlists can no longer hang behind a TTS announcement, playlists beyond 511 tracks retain the full track index, and browser-started playback never writes the sentinel RFID `000000000000` into NVS | [`bug fix`](../../commits/master) |
| **Hardened M3U and folder parsing**: imported `.m3u`/`.m3u8` files now handle UTF-8 BOMs, blank lines, indented comments, CRLF, case-insensitive HTTP(S) URLs and paths relative to the playlist folder (including safe `.`/`..` normalization). Oversized lines/lists and unsupported entries are bounded instead of consuming arbitrary heap; an empty M3U stays empty instead of being decoded as audio. Recursive folder scans carry their state per request rather than in shared statics and never mistake a directory named `*.mp3` for a track. The REST writer accepts only validated files below `/Playlists`, rejects newline injection and writes through a temporary/backup file so an SD short-write preserves the previous playlist | [`bug fix`](../../commits/master) |
| **Reliable WebUI requests and settings state**: settings auto-saves are serialized in edit order, failed writes restore the authoritative device state, and a reconnect can no longer overwrite a pending local edit with stale values. HTTP failures and timeouts are surfaced instead of producing false success messages, empty successful REST responses no longer fail JSON parsing, overlapping progress requests are suppressed, and WebSocket reconnects clean up their timers and tolerate malformed messages | [`bug fix`](../../commits/master) |
| **Lower decoder load with more free internal heap**: the unused per-sample VU calculation is disabled and the IIR tone filter only runs while the equalizer is non-flat, leaving substantially more core-1 time for expensive AAC/M4A decoding and input handling. The I²S DMA queue uses 40 instead of 48 descriptors (about 232 ms at 44.1 kHz), returning roughly 8 KB of internal RAM while retaining about 46 ms more flash-stall protection than the old 32-descriptor setup | [`performance`](../../commits/master) |
| **Folder cards play subfolders too (recursive by default)**: assigning a card to a directory now plays every track in that folder *and* its subfolders, for **all** folder play modes (audiobook, all-tracks sorted/random, their loops, and random-subdirectory), not just the dedicated `*_RECURSIVE` modes. Previously a card on a folder ignored its subdirectories entirely — a folder of albums/CDs played nothing but the loose files in the top level. Depth is capped by the existing **playlist recursion depth** setting (default 2); the recursion machinery was already there, only the non-recursive modes were hard-limited to depth 0. Existing cards pick this up automatically — no re-assignment needed | [`bug fix`](../../commits/master) |
| Tag removal detected via consecutive-miss counter instead of a wall-clock timeout: pause after ~0.5 s, immune to phantom dropouts and task starvation. MFRC522 presence polling now distinguishes a genuinely empty field from a corrupted reply caused by RF noise, and cycles the antenna after removal so a falsely HALTed card can be detected again | [`1fad9cd`](../../commit/1fad9cd) |
| **Live volume limits**: changed speaker/headphone maximums take effect immediately and clamp the current volume when necessary; no restart is required | |
| **Stop instead of pause when the tag is removed** (General → options, sub-option of "Pause when RFID tag is removed"): pulling the card fully stops playback (clears the playlist, so the LEDs return to the idle animation) rather than just pausing. The audiobook resume point is now saved at the **exact** moment of removal — not just the last ~30 s periodic checkpoint — so re-applying the card continues right where you pulled it (non-position-saving content still starts over). In the related runtime pause mode, re-applying the same card after its track or playlist finished naturally starts it again instead of being mistaken for an in-progress re-apply. Persisted in NVS (`stopRfidRem`) and applied live without a reboot | [`dd3b4d7`](../../commit/dd3b4d7) · [`bug fix`](../../commits/master) |
...
| **OLED quick menu and command feedback** (`CMD_OLED_MENU`, command 194): assign the menu to button 6 (or any physical/IR button) in the web UI. One short press opens it, the volume encoder browses without changing volume, and the same button confirms. Under Settings → OLED, entries can be enabled/hidden and drag-sorted, the 1–30 s inactivity timeout is configurable, and the last selection can optionally be remembered. One consolidated status page shows playback/volume, battery, firmware, uptime, heap and SD health, with a deliberately blank second line; list rows also show live EQ, night-mode, Bluetooth mode and firmware-update state. The Equalizer entry opens its own encoder-driven submenu for Flat, Music, Speech and Voice Boost; the Bluetooth submenu selects Normal, Speaker or Headphones mode. Pressing again applies the selected value; a changed Bluetooth mode restarts the device through the existing safe mode-switch path. Firmware update reports **UP TO DATE** without reflashing when the latest release is already installed; actual updates and shutdown still require a second confirmation press. Play/Pause, Loop Track and Cycle EQ Profile commands additionally show a large two-second OLED overlay with the resulting state/profile. Long-press and rotary-modifier assignments are unchanged | [`feature`](../../commits/master) |
| **Cancellable OLED shutdown countdown**: every normal shutdown first shows `SHUTTING DOWN...` and a 3-second countdown; any physical button press cancels it and is consumed instead of triggering its assigned action. The delay is configurable live under Settings → OLED from 0 (immediate shutdown) to 30 seconds and is shared by button, web, MQTT, sleep-timer and inactivity shutdowns | [`feature`](../../commits/master) |
...
| **RTC die-temperature over MQTT**: the DS3231's internal temperature is now published on the `rtc_temperature` topic (°C, every 60 s alongside the `rtc` time), with a Home Assistant `temperature` sensor added to the auto-discovery | [`feature`](../../commits/master) |
...
| **Reliable USB cold starts with OLED + Neopixels**: on the Complete board, do **not** power an OLED breakout with onboard I²C pull-ups from the external header's `3.3V switched` pin while the WS2812 chain is on that same switched rail. With the rail initially off, the OLED then pulls SDA/SCL towards the unloaded/clamped switched supply and the PCA9555 cannot answer; because that expander controls PE114, the rail can never be enabled and the later SD error is only a symptom. Connect OLED VCC to the header's unswitched `3.3V` pin instead (or remove/isolate the breakout's pull-ups before using switched VCC); never bridge the two supply pins. Firmware additionally holds WS2812 DIN low immediately, preloads PE114 safely, retries the PCA9555/POWER writes instead of continuing into a misleading SD timeout, waits for rail settling, and starts the LED animation only after SD has mounted | [`bug fix`](../../commits/master) |
| **Self-healing peripheral startup + crash evidence**: the SD boot path is now a recovery state machine: failed mounts tear the driver down cleanly, retry at a conservative clock and, only when needed, power-cycle the peripheral rail before trying again. The shared external I²C bus checks SDA/SCL before its first transaction, generates the standard nine open-drain recovery clocks plus STOP for a stuck slave and restarts the controller after failed PCA9555 power writes. A dedicated 1 MiB flash partition finally stores ESP-IDF ELF core dumps; startup validates and reports a previous crash, while the authenticated `GET /coredump` / `DELETE /coredump` endpoints and the Info dialog let an operator download or clear it. SD/I²C recovery counters are exposed by `/info` for diagnosis. Installing the new partition table requires one full **USB flash**; an app-only OTA cannot create the core-dump partition | [`stability`](../../commits/master) |
...
| Lock web playback while an RFID tag is applied: an optional switch (Settings → General → RFID, NVS key `blockWebPlayTag`, default off) refuses to start playback from the web file browser while a tag physically sits on the reader (PN5180 presence detection, incl. the removal grace period). Web-started playback would replace the tag's playback and the tag-removal pause/stop/position logic would then act on content the tag never started. The refusal shows as a toast in the web UI | |
| Reverse-proxy / HTTPS support for the PWA offline page: service workers only run in a secure context, so over plain `http://<ip>` the offline fallback never registered and an installed iPhone PWA showed a black screen while the box slept. Served through an HTTPS reverse proxy the worker registers, and `sw.js` now also treats the proxy's 502/503/504 as offline (a proxy answers instead of failing the fetch), showing the reworked German "Box schläft" page with wake instructions and auto-reconnect. A ready-to-use SWAG/nginx site config lives in [`tools/swag/`](tools/swag/at1.subdomain.conf) (websocket upgrade, unlimited upload size, streaming, optional WebDAV host); the proxy also serves the same offline page itself via nginx `error_page 502/503/504`, so even a first visit without a registered service worker gets "Box schläft" instead of nginx's bare 502 | |
| Songs don't resume mid-track: tracks shorter than a configurable length (new **"Restart songs from the beginning up to (sec.)"** general setting, NVS key `shortTrackSec`, default 300 s = 5 min, 0 = off) count as songs (music-album card) rather than audiobook chapters — re-applying the card restarts the current song from its beginning while the track number is kept. Long chapter files keep resuming at the exact saved position. Decided when the resume point is persisted, using the known duration of the playing track | |
