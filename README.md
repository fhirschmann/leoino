...
| **Protected `/System` folder**: the Explorer highlights `/System` (magenta + lock badge, "protected" tooltip) and drops its **delete**/**rename** context-menu actions; the firmware also refuses a direct `DELETE /explorer?path=/System` (HTTP 403), so the system folder can't be wiped by accident or a stray API call. The file-sync mirror-delete pass skips `/System` **and** `/Playlists` (and everything inside them) as well | |
